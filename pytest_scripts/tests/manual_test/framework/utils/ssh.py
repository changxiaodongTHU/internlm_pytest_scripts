...
class SSH(Common):
    def __init__(self, hostname, port=22, username='root', pkey=None, password=None, default_env=None,
        ...
        self.exec_file = None
        self.term = term or {}
        self.eof = 'QA SSH EOF 2023042415'
        self.default_env = default_env
        self.regex = re.compile(r'QA SSH EOF 2023042415 (-?\d+)[\r\n]?')
        self.arguments = {
            'hostname': hostname,
            'port': port,
            'username': username,
    ...
    def _handle_command(self, command, environment):
        ...
        if not self.exec_file:
            self.exec_file = f'/tmp/qa_ssh.{uuid4().hex}'
    ...
    def _get_channel(self):
        ...
        while True:
            if self.channel.recv_ready():
                ...
                self.client.close()
                raise Exception('Wait qa_ssh response timeout')
            else:
                counter += 1
                time.sleep(0.1)
        return self.channel
